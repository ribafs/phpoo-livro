...
if($conn->sgbd == 'mysql'){
    $results = $cliente->pdo->prepare("SELECT * FROM clientes ORDER BY id LIMIT $start, $conn->regsPerPage");
}else if($conn->sgbd == 'pgsql'){
    $results = $cliente->pdo->prepare("SELECT * FROM clientes ORDER BY id LIMIT $conn->regsPerPage OFFSET $start");
}
